/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Makena Williamson & Rohan Kallur
 * Version     : 1.0
 * Date        : 11/19/22
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value)
    {
        ...
        while (x != nullptr)
        {
            if (key == x->key())
            {
                size_--;
                delete z;
                stringstream dup_key;
                dup_key << "Attempt to insert duplicate key '" << key << "'.";
                throw tree_exception(dup_key.str());
            }
            y = x;
            if (key < x->key())
            {
...
private:
    ...
    int diameter(Node<K, V> *node) const
    {
        ...
        else
        {
            ...
            int left_d = diameter(node->left);
            int right_d = diameter(node->right);
